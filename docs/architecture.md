...
각 마이크로서비스는 DDD 관점에서 독립된 바운디드 컨텍스트(인증, 주문, 결제, 카탈로그)로 구분되며, 애그리게잇과 이벤트를 통해 상호 작용합니다.  
...
> `CATALOG_ADMIN_TOKEN` 환경 변수가 설정되지 않으면 catalog-service와 API Gateway 모두 관리자 요청을 거부하도록 fail-closed 전략을 적용했습니다.  
